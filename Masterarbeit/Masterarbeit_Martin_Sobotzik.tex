...
\lettrine{T}{he} standard model of particle physics (SM) is an extraordinary successful theory which describes three out of four fundamental forces in nature. The predictions of this theory are extremely accurate within the reach of precision and energy currently available at modern experiments. 
...
Chapter \ref{cha:SM} will present a short introduction to the standard model of particle physics. Also, Bhabha scattering will be explained briefly followed by Bhabha kinematics at Belle II. 
...
Since, the ECL will be used as a tool to select Bhabha events, one has to be sure that it send out a trigger signal in each event which is taken into account. Therefore, the trigger and data acquisition system will be described briefly in chapter \ref{sec:TDAS}.  

A very briefly introduction to the analysis framework at Belle II, BASF2, will be given in chapter \ref{sec:Tools}.
...
Last but not least, a short summary and conclusion will be given in chapter \ref{cha:Conclusion}.
\newline 
...
\caption[Bhabha Feynman Diagrams]{The two Bhabha process Feynman diagrams. The left diagram describes the annihilation and pair production process, while the right diagram describes a classic electromagnetic scattering process.}
\label{fig:feynman}
...
\end{figure}
 ...
 In the innermost of the detector, three tracking sub-detectors are located, surrounding the IR. These sub-detectors are in a axial magnetic field of $1.5\,\textrm{T}$, provided by a solenoid, to be able to reconstruct the trajectories (tracks) of charged particles. 
...
\end{minipage}
	...
	\caption[ARICH]{Left: Illustration of the working principle of the ARICH detector. The yellow Cerenkov ring on the photon detector is produced by a $\pi$ meson, the green ring by a $K$ meson. Right: The radiator is shown in more detail. The radiator consists of two aerogel layers with different reflective index. \cite{TORASSA} }
\label{fig:ARICH}
\end{figure}

In figure \ref{fig:ARICH} the working principle of the ARICH detector is shown. A charged particle passes through two layers of an aerogel radiator with different reflective indexes and emits Cerenkov photons under a Cerenkov angle $\theta_C$. Behind the radiator, an extension volume, for the Cerenkov rings to form, is located. At a distance of $20\,\textrm{cm}$ behind the radiator is the photon detector.\cite{B2TR} Once the Cerenkov ring is reconstructed, the radius of the ring can be determined and, knowing the distance and the radius, the Cerenkov angle can be calculated.
...
Since the entries in the \texttt{vpho:ECLObject} list are sorted by energy, the first daughter of the reconstructed virtual Bhabha photon always has a higher energy than the second daughter. The first daughter will be referred to as \texttt{HclE} and the second daughter as \texttt{LclE}. You can see that the \texttt{HclE} daughter has always a higher energy than the \texttt{LclE} daughter in figure \ref{fig:clEDiff}. Here the clusterE(\texttt{LclE}) is subtracted from the clusterE(\texttt{HclE}) and only positive values remain. Therefore, \texttt{HclE} is the daughter with the higher energy.
...
In this work the tracking efficiency of the innermost detectors of Belle II was presented. 
...
In the barrel, unfortunately, no noticeable improved of the tracking efficiency could be observed.
