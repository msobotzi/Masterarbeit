...
\captionof{figure}[Bhabha Feynman Diagrams]{The two Bhabha process Feynman digrams. The left diagram describes the annihilation process, while the right diagram describes a classic electromagnetic scattering process.}
\label{fig:feynman}
\end{center}
...
In figure \ref{fig:CrossSectionElPos} on the left, the differential cross section for electrons is plotted. \textcolor{red}{beschreibe plot (die meisten werden kaum abgelenkt)} Equation \ref{eq:bhabhaCross} is also true for positron but since they are moving in the opposite direction, $\theta$ in the equation has to be replaced by $\pi - \theta$. The result can be found in figure \ref{fig:CrossSectionElPos} on the right.
...
Figure \ref{fig:CrossSectionBoth} shows the combination of the differantial cross section for electrons and positrons.


\subsection{Bhabha Kinematics At Belle II}
\label{sec:Kinematics}

At Belle II the electron and the positron beams have different energies. Also, they are hitting each other under an angle. Therefore, it is interesting to look at the kinematics in the lab system of Belle II.
...
The same steering file is used for data, MC and MC on GridKa. This file is located on KEK at:
\newline 
...
In equation \ref{eq:EffError}, $\epsilon$ is the calculated efficiency and $n$ is the total number of entries.
...
\textcolor{red}{plot fuer mc and data was No. ausgewaehlte kandidaten pro events zeigt und damit auch die totale zahl der events nach der selection.}


In figure \textcolor{red}{Zahl der Figure}, you can see the number of candidates per event after the selection.
...
\subsection{Random Mixed Efficiency}
