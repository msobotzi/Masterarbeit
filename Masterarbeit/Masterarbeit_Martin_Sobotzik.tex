...
Figure \ref{fig:cb2bData} shows the same plots as figure \ref{fig:cb2bMC} but with phase2 data. The peaks at $|\Delta \phi _{\textrm{True(pred)} - \textrm{reco}}| \approx 1.6$ are created when the \textit{tag} particle is reconstructed as the wrong charged particle. For example, the \textit{tag} particle is an electron but it was reconstructed as a positron then  $\phi_{\textrm{pred,b2b}}$ is calculated under the assumption that the \textit{probe} particle is an electron.
...
Similar to the phase2 data electron tracking efficiency, there are two vertical efficiency drops on the phase3 data tracking efficiency at $\theta_{\textrm{pred,b2b}} \approx 2.05$ and 2.15 with a predicted azimuthal angle of $-0.8 \lesssim \phi_{\textrm{pred,b2b}} \lesssim 0.8$.
